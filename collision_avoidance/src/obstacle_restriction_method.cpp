...
Point ORM::subgoalSelector(const Point & prefered_goal, const std::vector<Point> & L)
{
    ...
    for (size_t i = 1; i * degree_per_index < max_change_in_direction_; i++)
    {
        for (int j = -1; j < 2; j += 2)
        {
            int current_index = mod(wanted_index + (i * j), L.size());
            ...
            for (int k = -1; k < 2; k += 2)
            {
                ...
                if (L[next_to_index].x == 0 && L[next_to_index].y == 0)
                {
                    ...
                    Point subgoal = Point::getPointFromVectorDegrees(direction, distance);
                ...
                else if (Point::getDistance(L[current_index], L[next_to_index]) > 2 * radius_)
                {
                    ...
                    Point subgoal = Point::getMidpoint(L[current_index], L[next_to_index]);
